...
public:
    int maxWalls(vector<int>& r, vector<int>& d, vector<int>& w) {
        ...
        for (int i = 0; i < n; i++) rd[i] = {r[i], d[i]};
        ...
        while (i < w.size() && j < n) {
            if (w[i] < rd[j].first) walls.push_back(w[i++]);
            else if (w[i] == rd[j].first) base++, i++;
            else j++;
        }
        while (i < w.size()) walls.push_back(w[i++]);
        ...
        int idx = 0;
        while (idx < walls.size() && walls[idx] < rd[0].first) idx++;
        ...
        for (int k = 1; k < n; k++) {
            int s = idx;
            while (idx < walls.size() && walls[idx] < rd[k].first) idx++;
            int e = idx;
